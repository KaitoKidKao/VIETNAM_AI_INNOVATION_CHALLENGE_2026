...
class ValidationService:
    ...
    def validate(procedure_id: str, form_data: Dict[str, Any]) -> ValidationResponse:
        ...
        summary = (
            "Hồ sơ của bạn đã đạt các kiểm tra sơ bộ."
            if is_valid
            else "Phát hiện một số lỗi cần khắc phục trước khi nộp hồ sơ."
        )

        return ValidationResponse(
            procedure_id=procedure_id, is_valid=is_valid, findings=findings, summary_message=summary
        )
