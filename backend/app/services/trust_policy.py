...
class TrustPolicy:
    def metadata_for(
        ...
        is_current = (
            pack.effective_from is None or pack.effective_from <= date.today()
        ) and (pack.effective_to is None or pack.effective_to >= date.today())
        can_verify = (
        ...
        if pack.demo_pack and requested_state == TrustState.NEED_MORE_INFORMATION:
            ...
            trust_state = (
                requested_state if can_verify else TrustState.OFFICIAL_REVIEW_REQUIRED
            )
        ...
        return TrustMetadata(
            ...
            if can_verify and not pack.demo_pack
            else None,
            review_gate=review_gate,
    ...
    def needs_more_information(
        self, review_gate: ReviewGate | None = None
    ) -> TrustMetadata:
        return TrustMetadata(
            trust_state=TrustState.NEED_MORE_INFORMATION,
            review_gate=review_gate,
