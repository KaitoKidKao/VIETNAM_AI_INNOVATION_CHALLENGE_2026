...
class IntakeResponse(BaseModel):
    ...
    trust_state: str = Field(
        ..., description="verified_guidance | need_more_information | official_review_required"
    )
    required_clarifications: List[str] = Field(
        default_factory=list, description="Pending clarifying questions"
    )
    sources: List[Dict[str, str]] = Field(
        default_factory=list, description="Citations and source references"
    )
