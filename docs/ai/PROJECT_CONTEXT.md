...
- D-006 chấp thuận target RAG/knowledge release, provider-neutral LLM, approved data adapter, PII boundary, widget contract và topology deploy. Backend chỉ tạo port/fallback; từng capability chỉ được tích hợp sau Task Record và evidence riêng. D-012 chấp thuận Cloud Run backend-only production-disabled; chưa có URL cho đến khi candidate smoke đạt.
...
| Deploy/demo runtime | Cloud Run backend-only production-disabled được D-012 chấp thuận; chưa provision URL, hosting, secret hoặc CD | D-012 Accepted; candidate smoke là gate |
